<?php  
	...
	if ($file_tmp !== "") {
		if (move_uploaded_file($file_tmp, '../videos/'.$file)) {
			...
			$update = $connection->query("UPDATE video SET title='$edit_title', video='$file' WHERE id='$update_id'");
		...
		$update = $connection->query("UPDATE video SET title='$edit_title' WHERE id='$update_id'");
